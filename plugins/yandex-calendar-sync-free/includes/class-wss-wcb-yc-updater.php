<?php
/**
 * WordPress updater for WSS Yandex Calendar Free.
 *
 * @package WSS_WCB_YC
 */
...
final class WSS_WCB_YC_Updater {
	private const PRODUCT_ID      = 'wss_yandex_calendar_sync_free';
	private const UPDATE_ENDPOINT = 'https://website-support.ru/wp-json/wss-updates/v1/check';
	private const TRANSIENT_KEY   = 'wss_wcb_yc_free_update_info';
	...
	public function plugins_api_info( $result, string $action, object $args ) {
		...
		return (object) array(
			'name'              => 'WSS Yandex Calendar for WooCommerce Bookings',
			'slug'              => dirname( $this->plugin_basename ),
			...
			'short_description' => __( 'Синхронизация WooCommerce Bookings с Яндекс.Календарём.', 'wss-wcb-yandex-calendar' ),
			'sections'          => array(
				'description' => __( 'Плагин отправляет будущие бронирования WooCommerce Bookings в Яндекс.Календарь через CalDAV.', 'wss-wcb-yandex-calendar' ),
			),
