<?php
/**
 * WordPress updater for Delivery Zones Free.
 *
 * @package YDZS
 */
...
final class YDZS_Updater {
	private const PRODUCT_ID      = 'ydzs_free';
	private const UPDATE_ENDPOINT = 'https://website-support.ru/wp-json/wss-updates/v1/check';
	private const TRANSIENT_KEY   = 'ydzs_free_update_info';
	...
	public function plugins_api_info( $result, string $action, object $args ) {
		...
		return (object) array(
			'name'              => 'Delivery Zones on Map for WooCommerce',
			'slug'              => dirname( $this->plugin_basename ),
			...
			'short_description' => __( 'Доставка WooCommerce по нарисованным зонам на карте.', 'ydzs' ),
			'sections'          => array(
				'description' => __( 'Плагин рассчитывает доставку WooCommerce по зонам на карте, поддерживает правила стоимости и проверку адреса.', 'ydzs' ),
			),
